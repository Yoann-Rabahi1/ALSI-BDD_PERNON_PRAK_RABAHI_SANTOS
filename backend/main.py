...
# 1. Routes Système & Authentification
...
async def signup_full(data: UserSignup, db: db_dependency):
    try:
        new_user = models.CompteUser(mail=data.mail, mot_de_passe=data.mot_de_passe, role=data.role)
        db.add(new_user)
        db.flush() 
        if data.role == "client":
...
# 2. Routes "Utilisateurs" (CompteUser)
...
# 3. Routes "Propriétaires" (Proprietaire)
