...
const AddAnimal = () => {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            ...
            try {
                // On récupère le profil proprio via l'id_user (11 par exemple) 
                // pour obtenir l'id_proprietaire réel (1 par exemple)
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await api.post('/animaux/', {
                ...
                // Conversion sécurisée : si vide on envoie null, sinon on cast
                age: formData.age !== '' ? parseInt(formData.age) : null,
                poids_kg: formData.poids_kg !== '' ? parseFloat(formData.poids_kg) : null,
                id_proprietaire: idProprio
            });

            alert("Félicitations ! Votre compagnon a été ajouté. 🐾");
            setFormData({ nom_animal: '', espece: '', race: '', age: '', poids_kg: '' });
        } catch (err: any) {
            console.error(err);
            alert(err.response?.data?.detail || "Erreur lors de l'ajout de l'animal.");
        } finally {
    ...
    if (status.error) return <div className="error-msg">⚠️ {status.error}</div>;
